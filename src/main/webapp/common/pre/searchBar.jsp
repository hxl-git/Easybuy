...
<div class="top">
    ...
    <div class="search">
        <form action="${ctx}/queryLikeProductList.html" method="post">
            <input type="text" value="${name}" name="keyWord" class="s_ipt">
        ...
        <div class="car_bg">
            ...
            <ul class="cars">
                <c:forEach items="${sessionScope.cart.items}" var="temp">
                    <li>
                        <div class="img"><a href="${ctx}/queryProductDeatil.html.a?id=${temp.product.id}"><img src="${ctx}/files/${temp.product.fileName}" width="58" height="58" /></a></div>
                        <div class="name"><a href="${ctx}/queryProductDeatil.html.a?id=${temp.product.id}">${temp.product.name}</a></div>
            ...
            <c:if test="${sessionScope.easybuyUserLogin!=null}">
                <div class="price_a"><a href="${ctx}/toSettlement.html">去结算</a></div>
            </c:if>
            <!--End 购物车已登录 End-->
        </div>
    </div>
